...
import { ArrowLeft, CalendarDays, ChevronRight, CirclePlus, MapPin, RefreshCw, Waves } from "lucide-react";
...
type Props = { userId: string; onBack: () => void; onSelectSauna: (sauna: Sauna) => void; onCreatePost: () => void };
...
export function VisitedSaunasScreen({ userId, onBack, onSelectSauna, onCreatePost }: Props) {
  ...
  return <section className="saved-posts-screen">
    ...
    : items.length === 0 ? <div className="saved-posts-empty"><div className="saved-posts-empty-icon"><MapPin /></div><strong>行ったサウナはまだありません</strong><p>サ活を投稿すると、訪れた施設が自動で追加されます。</p><button type="button" className="empty-state-action" onClick={onCreatePost}><CirclePlus aria-hidden="true" />最初のサ活を記録する</button></div>
